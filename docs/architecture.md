...
- **Ввод и письмо пишутся вместе**, когда генерация дошла до конца, поэтому
  сохранённая форма всегда описывает сохранённое письмо.
- **`tone` — текст, а не enum.** Тон хранится, чтобы «Try Again» его
  сохранил. Убрать тон из кода можно без миграции: старые строки тогда
  читаются как тон по умолчанию.
- **Индексы:**
...
Планы и фичи настраиваются в дашборде Clerk (Billing). Их slug'и должны
совпадать с `billing.catalog.ts`, а у каждой фичи должен быть включён
«Include in JWT»: доступ проверяется через `has()` по токену, и фича без
этого флага для пользователя просто не существует.
...
- **Деплой** — `railway.json`: команды сборки, pre-deploy и старта,
  `drainingSeconds` и политика рестарта. Postgres, Redis и переменные
  окружения настраиваются в Railway.
