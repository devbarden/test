...
src/
  ...
  components/
    ui/              дизайн-система: Button, IconButton, Field/Input/TextArea,
                     Panel, StepProgress, Spinner, Alert, Toast, CopyButton
    layout/          хедер, контейнер страницы, статус-страница
  features/
    applications/    модель письма, схема формы, хранилище в браузере
    generation/      протокол, клиент потока, конечный автомат генерации
    editor/          экран генерации: форма, панель письма, орб ожидания
    dashboard/       экран списка: карточки, empty state
    goal/            прогресс к цели: индикатор в хедере, баннер
  server/generation/ прокси к Generation API: SSE-парсер, клиент, промпт,
                     rate limit, обработчик роута
  routes/            тонкие файлы роутов, которые только собирают фичи
```

Зависимости идут в одну сторону: `routes → features → components → styles`.
Компоненты из `components/` ничего не знают о письмах и авторизации. Хедер,
например, получает счётчик и меню аккаунта через слоты.
...
- **Таймауты.** Отдельно ждём первый байт (30 с) и отдельно простой
  потока (20 с). Общего таймаута нет: длинное письмо, которое продолжает
  писаться, — нормальная ситуация.
...
- **Rate limit на пользователя.** Лимит upstream — 6 запросов в минуту на
  *токен*, то есть на всех пользователей деплоя сразу. Поэтому у каждого
  ...
  жмёт «Try Again», не блокирует остальных. Лимит хранится в памяти, этого
  достаточно для одного инстанса. При горизонтальном масштабировании его
  надо вынести в Redis с тем же интерфейсом `consume`.
