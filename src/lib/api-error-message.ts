...
export function apiErrorMessage(error: ApiError): string {
	switch (error.code) {
		...
		case 'quota_exceeded':
			return 'You have reached today’s limit of generated letters. Come back tomorrow.'
		case 'generation_in_progress':
			return 'A letter is already being written in another tab. Wait for it to finish.'
		case 'application_limit_reached':
			return 'You have reached the maximum number of saved applications. Delete a few to make room.'
		case 'interrupted':
