import { useEntitlements } from '@/client/features/billing/hooks/use-entitlements'
import { withForm } from '@/client/kit/form'
import type { SegmentedOption } from '@/client/kit/segmented-control'
import { INPUT_LIMITS } from '@/domain/applications/application-input'
			...
			>
				...
				{entitlements.letterTones && (
					<form.AppField name="tone">
						{(field) => <field.SegmentedField label="Tone" options={TONE_OPTIONS} />}
					</form.AppField>
				)}
				...
				<form.Subscribe selector={(state) => [state.isValid, state.isSubmitting] as const}>
					{([isValid, isSubmitting]) => (
						<SubmitButton disabled={!isValid} hasLetter={hasLetter} isGenerating={isGenerating || isSubmitting} />
					)}
				</form.Subscribe>
			</form>
		)
